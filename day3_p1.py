...
def find_neighbors(arr, i, j):
    """
    returns a list of neighbors of arr[i][j]
    """
    ...
    for x in range(i-1, i+2):
        for y in range(j-1, j+2):
            if x >= 0 and x < m and y >= 0 and y < n and (x != i or y != j):
...
def find_sum(arr):
    """
    for each elt, if elt is a integer, look for symbol in its adjacency matrix 
    if symbol found, find whole number by finding its adjacent integers in the same row 
    add whole part number to sum 
    """
    ...
    for i in range(m):
        ...
        while j < n:
            if arr[i][j].isdigit():
                ...
                # look for symbol 
                neighbors = find_neighbors(arr, i, j)
                for neighbor_value in neighbors:
                    if (not neighbor_value.isdigit() and neighbor_value != "."):
                        is_part_number = True
                        break
                if is_part_number:
                    #look for rest of number 
                    part_number_s = arr[i][j]

                    # extend left
                    temp_j = j 
                    while temp_j-1 >= 0:
                        if arr[i][temp_j-1].isdigit():
                            part_number_s = arr[i][temp_j-1] + part_number_s
                            temp_j -= 1
                        else:
                            break
                    
                    #extend right - move j forward to dedup 
                    while j+1 < n:
                        if arr[i][j+1].isdigit():
                            part_number_s += arr[i][j+1]
                            j += 1
                        else:
                            break 
                    
                    result += int(part_number_s)
                
            j += 1
    return result 

# 4361
print(find_sum(example_arr))
...
arr = [[char for char in line] for line in input.split("\n")]
print(find_sum(arr))