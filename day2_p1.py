...
# which games would have been possible if the bag had been loaded with only 
# 12 red cubes, 13 green cubes, and 14 blue cubes?
def filter_games(arr):
    game_sum = 0 
    max_per_color = {"blue": 14, "red": 12, "green": 13}
    ...
    for i, game in enumerate(arr):
        ...
        # 1. get number each color appears in current roll
        # game_set: '3 blue, 4 red'
        for game_set in game_sets: 
            rolls = game_set.split(', ')

            # roll: '3 blue'
            for roll in rolls:
                value, color = roll.strip().split(' ')
                if int(value) > max_per_color[color]:
                    game_valid = False 
                    break

        # 3. if game is valid, add game number to sum 
        if game_valid:
            game_sum += (i+1)

    return game_sum 

# print(filter_games(example_arr)) # expect 8
print(filter_games(arr))