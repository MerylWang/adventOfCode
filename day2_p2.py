# so for each game, get the max per color across all pulls 
...
example_arr = example_input.splitlines()

def get_total_power(arr):
    """
    power of a game:= min number of each color cubes required for valid game

    return: sum of power of all games 
    """
    sum_of_powers = 0 

    for game in arr:
        min_cubes_needed = {'red': 0, 'blue': 0, 'green': 0}

        #  '3 blue, 4 red; 1 red, 2 green, 6 blue; 2 green'
        game_str = game.split(":")[1].strip() 

         #  ['3 blue, 4 red', '1 red, 2 green, 6 blue', '2 green']
        pulls_arr = game_str.split(";")

        for pull in pulls_arr:
            # pull: '3 blue, 4 red'
            for roll in pull.split(","):
                # roll: '3 blue'
                value, color = roll.strip().split(" ")
                min_cubes_needed[color] = max(min_cubes_needed[color], int(value))
        
        game_power =  reduce(lambda x, y: x * y, min_cubes_needed.values())
...
# expected: 2286
# powers: 48, 12, 1560, 630, and 36
print(get_total_power(example_arr))     
...
print(get_total_power(input_arr))
