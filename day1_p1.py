...
def find_sum(arr):
    ...
    finds coordinate of each line (first and last number string concat)
    and returns the numerical sum of all coordinates
    """
    result = 0 


    for word in arr:
        # find first and last number in word 

        # strip letters O(k), then take first and last number O(1)

        # or, iterate & keep tab of first and last number O(k)
        numbers = ''.join(char for char in word if char.isdigit())
